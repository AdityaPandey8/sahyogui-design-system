...
const currentNgo = ngos[0]; // Simulate logged-in NGO
...
export default function DashboardNGO() {
  ...
  // Issues assigned to this NGO or unclaimed
  ...
  const topIssue = useMemo(() =>
    [...ngoIssues].sort((a, b) => b.aiPriorityScore - a.aiPriorityScore)[0],
    [ngoIssues]
  );
  ...
  const handleAutoAssign = () => {
    ...
    const randomIssue = unassigned[0];
    const randomVol = available[Math.floor(Math.random() * available.length)];
    setIssueList((prev) => prev.map((i) => i.id === randomIssue.id ? { ...i, assignedNgo: currentNgo.id, status: "Verified" as const } : i));
    toast.success(`Auto-assigned ${randomVol?.name} to "${randomIssue.title}"`);
  };
  ...
  // Simulated volunteer matching scores
    ...
    <div className={`min-h-screen bg-background ${crisisMode ? "ring-2 ring-danger ring-inset" : ""}`}>
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
          ...
          <MetricCard icon={AlertTriangle} label="Active" value={stats.active} trend={{ direction: "up", value: "+1" }} />
          <MetricCard icon={CheckCircle} label="Completed" value={stats.completed} trend={{ direction: "up", value: "+2" }} />
          <MetricCard icon={Clock} label="Avg Response" value={stats.avgResponse} trend={{ direction: "down", value: "-8m" }} />
          <MetricCard icon={Users} label="Volunteers" value={stats.activeVols} />
        </div>

        {/* AI + Map */}
        <div className="grid gap-4 lg:grid-cols-2">
          {topIssue && <AIPriorityCard issue={topIssue} />}
          <HeatmapPlaceholder issues={ngoIssues} volunteers={volunteers} size="lg" />
        </div>

        {/* Issue Management */}
        <div>
          <h2 className="mb-3 text-sm font-bold flex items-center gap-1.5"><Brain className="h-4 w-4 text-primary" /> Issue Management</h2>
          <FilterBar
            search={search}
        ...
        <div>
          <h2 className="mb-3 text-sm font-bold flex items-center gap-1.5"><UserCheck className="h-4 w-4 text-primary" /> Smart Volunteer Matching</h2>
          <div className="grid gap-2 sm:grid-cols-2">
            {matchedVolunteers.slice(0, 4).map((m, i) => (
              <VolunteerMatchCard
                ...
                distance={m.distance}
                highlight={i === 0}
                onAssign={() => toast.success(`${m.volunteer.name} assigned!`)}
              />
            ))}
          </div>
        </div>
        ...
        {/* Collaboration */}
        <div>
          <h2 className="mb-3 text-sm font-bold flex items-center gap-1.5"><Handshake className="h-4 w-4 text-primary" /> Collaboration</h2>
          <div className="grid gap-2 sm:grid-cols-3">
            {ngos.filter((n) => n.id !== currentNgo.id).map((ngo) => (
              <div key={ngo.id} className="rounded-lg border bg-card p-3">
                <p className="text-sm font-semibold">{ngo.name}</p>
                <p className="text-xs text-muted-foreground mb-2">{ngo.focusArea}</p>
                <div className="flex gap-1">
                  <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => toast.success(`Help requested from ${ngo.name}`)}>Request Help</Button>
                  <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => toast.success(`Resources shared with ${ngo.name}`)}>Share Resources</Button>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Communication */}
        <div>
          <h2 className="mb-3 text-sm font-bold flex items-center gap-1.5"><Send className="h-4 w-4 text-primary" /> Broadcast to Volunteers</h2>
        ...
        {/* Analytics */}
        <div>
          <h2 className="mb-3 text-sm font-bold flex items-center gap-1.5"><Trophy className="h-4 w-4 text-warning" /> Performance Analytics</h2>
          <div className="grid gap-3 sm:grid-cols-4">
            <div className="rounded-lg border bg-card p-4 text-center">
              <p className="text-2xl font-bold tabular-nums">{currentNgo.issuesHandled}</p>
              <p className="text-xs text-muted-foreground">Issues Handled</p>
            </div>
            <div className="rounded-lg border bg-card p-4 text-center">
              <p className="text-2xl font-bold tabular-nums text-success">{currentNgo.successRate}%</p>
              <p className="text-xs text-muted-foreground">Success Rate</p>
            </div>
            <div className="rounded-lg border bg-card p-4 text-center">
              <p className="text-xl font-bold">{currentNgo.avgResponseTime}</p>
              <p className="text-xs text-muted-foreground">Avg Response</p>
            </div>
            <div className="rounded-lg border bg-card p-4 text-center">
              <p className="text-2xl font-bold tabular-nums text-primary">4,200</p>
        ...
        {/* Resource Allocation */}
        <div>
          <h2 className="mb-3 text-sm font-bold flex items-center gap-1.5"><ActivityIcon className="h-4 w-4 text-primary" /> Resource Status</h2>
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-lg border bg-card p-4 text-center">
              <p className="text-2xl font-bold tabular-nums">{ngoIssues.filter((i) => i.status !== "Solved").length}</p>
              <p className="text-xs text-muted-foreground">Open Issues</p>
            </div>
            <div className="rounded-lg border bg-warning/10 border-warning/30 p-4 text-center">
              <p className="text-sm font-bold text-warning">Need 3 more volunteers in Area B</p>
              <p className="text-xs text-muted-foreground">Resource Gap</p>
            </div>
          </div>
        </div>
        ...
        {/* Alerts */}
        <div>
          <h2 className="mb-2 text-sm font-bold">Notifications</h2>
          <div className="grid gap-2 sm:grid-cols-2">
